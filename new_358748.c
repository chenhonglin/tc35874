/*		
* TC358748드라이버 새버전- 기존 V4L2부분 보존, Register부분만 수정.
* TC358748드라이버 간소화버전. i2c_rd를 최소한으로 줄임.
				                         ...
				                         struct v4l2_dv_timings *timings)
{
	...
	//HC bt->interlaced = i2c_rd8(sd, VI_STATUS1) & MASK_S_V_INTERLACE ? V4L2_DV_INTERLACED : V4L2_DV_PROGRESSIVE;
	bt->interlaced=0;
	
	//HC width = ((i2c_rd8(sd, DE_WIDTH_H_HI) &0x1f) << 8) + i2c_rd8(sd, DE_WIDTH_H_LO);
	//HC height = ((i2c_rd8(sd, DE_WIDTH_V_HI) &0x1f) << 8) + i2c_rd8(sd, DE_WIDTH_V_LO);
	//HC frame_width = ((i2c_rd8(sd, H_SIZE_HI) &0x1f) << 8) + i2c_rd8(sd, H_SIZE_LO);
	//HC frame_height = (((i2c_rd8(sd, V_SIZE_HI) &0x3f) << 8) + i2c_rd8(sd, V_SIZE_LO)) / 2;
	width=1280;
	height=720;
	frame_width=1650;
	frame_height=750;
	...
	frame_interval = 167;//((i2c_rd8(sd, FV_CNT_HI) &0x3) << 8) + i2c_rd8(sd, FV_CNT_LO);
...
static void tc358748_delayed_work_enable_hotplug(struct work_struct *work)
{
	...
	//743 i2c_wr8_and_or(sd, HPD_CTL, ~MASK_HPD_OUT0, MASK_HPD_OUT0);
	/*hainh 
...
static void tc358748_set_hdmi_hdcp(struct v4l2_subdev *sd, bool enable)
{
	...
	//743 i2c_wr8_and_or(sd, HDCP_REG1,~(MASK_AUTH_UNAUTH_SEL | MASK_AUTH_UNAUTH),MASK_AUTH_UNAUTH_SEL_16_FRAMES | MASK_AUTH_UNAUTH_AUTO);

	//743 i2c_wr8_and_or(sd, HDCP_REG2, ~MASK_AUTO_P3_RESET,SET_AUTO_P3_RESET_FRAMES(0x0f));
	...
	//743 i2c_wr8_and_or(sd, HDCP_REG3, ~KEY_RD_CMD, enable ? KEY_RD_CMD : 0);
	//743 i2c_wr8_and_or(sd, HDCP_MODE, ~(MASK_AUTO_CLR | MASK_MODE_RST_TN), enable ?  (MASK_AUTO_CLR | MASK_MODE_RST_TN) : 0);
	...
	//743 i2c_wr8_and_or(sd, BSTATUS1, ~MASK_MAX_EXCED,enable ? 0 : MASK_MAX_EXCED);
	//743 i2c_wr8_and_or(sd, BCAPS, ~(MASK_REPEATER | MASK_READY),enable ? 0 : MASK_REPEATER | MASK_READY);
}
...
static void tc358748_disable_edid(struct v4l2_subdev *sd)
{
	 ...
	 * register DDC_CTL */
	//743 i2c_wr8_and_or(sd, HPD_CTL, ~MASK_HPD_OUT0,0x0);
}
...
static void print_avi_infoframe(struct v4l2_subdev *sd)
{
	...
	//no return: i2c_rd(sd, PK_AVI_0HEAD, buffer, HDMI_INFOFRAME_SIZE(AVI));
...
static void tc358748_reset_phy(struct v4l2_subdev *sd)
{
	...
	//743 i2c_wr8_and_or(sd, PHY_RST, ~MASK_RESET_CTRL, 0);
	//743 i2c_wr8_and_or(sd, PHY_RST, ~MASK_RESET_CTRL, MASK_RESET_CTRL);
}

static void tc358748_reset(struct v4l2_subdev *sd, uint16_t mask)
{
	u16 sysctl = i2c_rd16(sd, SYSCTL);
	printk("TC358748DEV: tc358748_reset : sysctl = %u ", sysctl);
	
	i2c_wr16(sd, SYSCTL, SYSCTL_SRESET_MASK);
	udelay(10);
	i2c_wr16(sd, SYSCTL, 0);
	//743 i2c_wr16(sd, SYSCTL, sysctl | mask);
	//743 i2c_wr16(sd, SYSCTL, sysctl & ~mask);
}

static inline void tc358748_sleep_mode(struct v4l2_subdev *sd, bool enable)
{
	//743 i2c_wr16_and_or(sd, SYSCTL, ~MASK_SLEEP, enable ? MASK_SLEEP : 0);
}
...
static inline void enable_stream(struct v4l2_subdev *sd, bool enable)
{
	...
	if (enable) {
		 ...
		 * LP11 state. */
		//743 i2c_wr32(sd, TXOPTIONCNTRL, 0);
		/* Set to continuous mode to trigger LP11->HS transition */
		//743 i2c_wr32(sd, TXOPTIONCNTRL, MASK_CONTCLKMODE);
		/* Unmute video */
		//743 i2c_wr8(sd, VI_MUTE, MASK_AUTO_MUTE);
	} else {
		/* Mute video so that all data lanes go to LSP11 state.
		 * No data is output to CSI Tx block. */
		//743 i2c_wr8(sd, VI_MUTE, MASK_AUTO_MUTE | MASK_VI_MUTE);
	}

	mutex_lock(&state->confctl_mutex);
	//743 i2c_wr16_and_or(sd, CONFCTL, ~(MASK_VBUFEN | MASK_ABUFEN), enable ? (MASK_VBUFEN | MASK_ABUFEN) :0x0);
...
static void tc358748_set_ref_clk(struct v4l2_subdev *sd)
{
	...
	//743 i2c_wr8(sd, SYS_FREQ0, sys_freq &0x00ff);
	//743 i2c_wr8(sd, SYS_FREQ1, (sys_freq &0xff00) >> 8);

	//743 i2c_wr8_and_or(sd, PHY_CTL0, ~MASK_PHY_SYSCLK_IND,(pdata->refclk_hz == 42000000) ? MASK_PHY_SYSCLK_IND :0x0);

	fh_min = pdata->refclk_hz / 100000;
	//743 i2c_wr8(sd, FH_MIN0, fh_min &0x00ff);
	//743 i2c_wr8(sd, FH_MIN1, (fh_min &0xff00) >> 8);

	fh_max = (fh_min * 66) / 10;
	//743 i2c_wr8(sd, FH_MAX0, fh_max &0x00ff);
	//743 i2c_wr8(sd, FH_MAX1, (fh_max &0xff00) >> 8);

	lockdet_ref = pdata->refclk_hz / 100;
	//743 i2c_wr8(sd, LOCKDET_REF0, lockdet_ref &0x0000ff);
	//743 i2c_wr8(sd, LOCKDET_REF1, (lockdet_ref &0x00ff00) >> 8);
	//743 i2c_wr8(sd, LOCKDET_REF2, (lockdet_ref &0x0f0000) >> 16);

	//743 i2c_wr8_and_or(sd, NCO_F0_MOD, ~MASK_NCO_F0_MOD,(pdata->refclk_hz == 27000000) ? MASK_NCO_F0_MOD_27MHZ :0x0);
}
static void tc358748_set_csi_color_space(struct v4l2_subdev *sd)
{
	...
	switch (state->mbus_fmt_code) {
		case MEDIA_BUS_FMT_UYVY8_1X16:
			...
			//743 i2c_wr8_and_or(sd, VOUT_SET2,~(MASK_SEL422 | MASK_VOUT_422FIL_100) &0xff,MASK_SEL422 | MASK_VOUT_422FIL_100);
			//743 i2c_wr8_and_or(sd, VI_REP, ~MASK_VOUT_COLOR_SEL &0xff,MASK_VOUT_COLOR_601_YCBCR_LIMITED);
			mutex_lock(&state->confctl_mutex);
			//743 i2c_wr16_and_or(sd, CONFCTL, ~MASK_YCBCRFMT,MASK_YCBCRFMT_422_8_BIT);
			mutex_unlock(&state->confctl_mutex);
			break;
		case MEDIA_BUS_FMT_RGB888_1X24:
			v4l2_info(sd, "%s: RGB 888 24-bit\n", __func__);
			//743 i2c_wr8_and_or(sd, VOUT_SET2,~(MASK_SEL422 | MASK_VOUT_422FIL_100) &0xff,0x00);
			//743 i2c_wr8_and_or(sd, VI_REP, ~MASK_VOUT_COLOR_SEL &0xff,MASK_VOUT_COLOR_RGB_FULL);
			mutex_lock(&state->confctl_mutex);
			//743 i2c_wr16_and_or(sd, CONFCTL, ~MASK_YCBCRFMT, 0);
			mutex_unlock(&state->confctl_mutex);
			break;
		default:
...
static void tc358748_set_csi(struct v4l2_subdev *sd)
{
	...
	i2c_wr32(sd, TCLK_TRAILCNT, pdata->tclk_trailcnt);
	i2c_wr32(sd, TXOPTIONCNTRL, (pdata->endpoint.bus.mipi_csi2.flags & V4L2_MBUS_CSI2_CONTINUOUS_CLOCK) ? MASK_CONTCLKMODE : 0);
	tc38764_debug_pattern_80(sd);
	
	
	//743 i2c_wr32(sd, LINEINITCNT, pdata->lineinitcnt);
	//743 i2c_wr32(sd, LPTXTIMECNT, pdata->lptxtimecnt);
	...
	//743 
	//743 
	//743 i2c_wr32(sd, HSTXVREGCNT, pdata->hstxvregcnt);
	...
	//743 
	//743 
	//743 i2c_wr32(sd, TCLK_HEADERCNT, pdata->tclk_headercnt);
	//743 
	//743 i2c_wr32(sd, THS_HEADERCNT, pdata->ths_headercnt);
	//743 i2c_wr32(sd, TWAKEUP, pdata->twakeup);
	//743 i2c_wr32(sd, TCLK_POSTCNT, pdata->tclk_postcnt);
	//743 i2c_wr32(sd, THS_TRAILCNT, pdata->ths_trailcnt);
	//743 i2c_wr32(sd, HSTXVREGCNT, pdata->hstxvregcnt);
	...
	//743 i2c_wr32(sd, TXOPTIONCNTRL, (pdata->endpoint.bus.mipi_csi2.flags & V4L2_MBUS_CSI2_CONTINUOUS_CLOCK) ? MASK_CONTCLKMODE : 0);
	//743 i2c_wr32(sd, STARTCNTRL, MASK_START);
	//743 i2c_wr32(sd, CSI_START, MASK_STRT);

	//743 i2c_wr32(sd, CSI_CONFW, MASK_MODE_SET |
	...
	//743 i2c_wr32(sd, CSI_CONFW, MASK_MODE_CLEAR | MASK_ADDRESS_CSI_ERR_HALT | MASK_TXBRK | MASK_QUNK);

	//743 i2c_wr32(sd, CSI_CONFW, MASK_MODE_SET | MASK_ADDRESS_CSI_INT_ENA | MASK_INTER);
}
...
static void tc358748_set_hdmi_phy(struct v4l2_subdev *sd)
{
	...
	// turn of physics
	//743 i2c_wr8_and_or(sd, PHY_EN, ~MASK_ENABLE_PHY,0x0);
	//743 i2c_wr8(sd, PHY_CTL1, SET_PHY_AUTO_RST1_US(1600) |
	// 		SET_FREQ_RANGE_MODE_CYCLES(1));
	//743 i2c_wr8_and_or(sd, PHY_CTL2, ~MASK_PHY_AUTO_RSTn,
	...
	//743 i2c_wr8(sd, PHY_CSQ, SET_CSQ_CNT_LEVEL(0x0a));
	//743 i2c_wr8(sd, AVM_CTL, 45);
	//743 i2c_wr8_and_or(sd, HDMI_DET, ~MASK_HDMI_DET_V, pdata->hdmi_detection_delay << 4);

	//743 i2c_wr8_and_or(sd, HV_RST, ~(MASK_H_PI_RST | MASK_V_PI_RST),
	//		(pdata->hdmi_phy_auto_reset_hsync_out_of_range ?
	//		 MASK_H_PI_RST : 0) |
	//		(pdata->hdmi_phy_auto_reset_vsync_out_of_range ?
	//		 MASK_V_PI_RST : 0));
	// turn on physics
	//743 i2c_wr8_and_or(sd, PHY_EN, ~MASK_ENABLE_PHY, MASK_ENABLE_PHY);
}

static void tc358748_set_hdmi_info_frame_mode(struct v4l2_subdev *sd)
{
	/* Default settings from REF_02, sheet "Source HDMI" */
	//743 i2c_wr8(sd, PK_INT_MODE, MASK_ISRC2_INT_MODE | MASK_ISRC_INT_MODE |
	//		MASK_ACP_INT_MODE | MASK_VS_INT_MODE |
	//		MASK_SPD_INT_MODE | MASK_MS_INT_MODE |
	//		MASK_AUD_INT_MODE | MASK_AVI_INT_MODE);
	//743 i2c_wr8(sd, NO_PKT_LIMIT,0x2c);
	//743 i2c_wr8(sd, NO_PKT_CLR,0x53);
	//743 i2c_wr8(sd, ERR_PK_LIMIT,0x01);
	//743 i2c_wr8(sd, NO_PKT_LIMIT2,0x30);
	//743 i2c_wr8(sd, NO_GDB_LIMIT,0x10);
}
...
static void tc358748_initial_setup(struct v4l2_subdev *sd)
{
	...
	//743 i2c_wr16_and_or(sd, SYSCTL, ~(MASK_CECRST | MASK_IRRST), (MASK_CECRST | MASK_IRRST));

	tc358748_reset(sd, MASK_CTXRST | MASK_HDMIRST);
	tc358748_sleep_mode(sd, false);

	//743 i2c_wr16(sd, FIFOCTL, pdata->fifo_level);

	tc358748_set_ref_clk(sd);

	//743 i2c_wr8_and_or(sd, DDC_CTL, ~MASK_DDC5V_MODE, pdata->ddc5v_delay & MASK_DDC5V_MODE);

	//743 i2c_wr8_and_or(sd, EDID_MODE, ~MASK_EDID_MODE, MASK_EDID_MODE_E_DDC);

	tc358748_set_hdmi_phy(sd);
	tc358748_set_hdmi_hdcp(sd, pdata->enable_hdcp);
	//AUD tc358748_set_hdmi_audio(sd);
	tc358748_set_hdmi_info_frame_mode(sd);

	/* All CE and IT formats are detected as RGB full range in DVI mode */
	//743 i2c_wr8_and_or(sd, VI_MODE, ~MASK_RGB_DVI, 0);

	//743 i2c_wr8_and_or(sd, VOUT_SET2, ~MASK_VOUTCOLORMODE, MASK_VOUTCOLORMODE_AUTO);
	//743 i2c_wr8(sd, VOUT_SET3, MASK_VOUT_EXTCNT);
}
...
static void tc358748_init_interrupts(struct v4l2_subdev *sd)
{
	...
	//743 	i2c_wr8(sd, i,0xff);

	//743 i2c_wr16(sd, INTSTATUS,0xffff);
}
...
static void tc358748_enable_interrupts(struct v4l2_subdev *sd, bool cable_connected)
{
	...
	//if (cable_connected) {
		//743 i2c_wr8(sd, SYS_INTM, ~(MASK_M_DDC | MASK_M_DVI_DET | MASK_M_HDMI_DET) &0xff);
		//743 i2c_wr8(sd, CLK_INTM, ~MASK_M_IN_DE_CHG);
		//743 i2c_wr8(sd, CBIT_INTM, ~(MASK_M_CBIT_FS | MASK_M_AF_LOCK | MASK_M_AF_UNLOCK) &0xff);
		//743 //AUD i2c_wr8(sd, AUDIO_INTM, ~MASK_M_BUFINIT_END);
		//743 i2c_wr8(sd, MISC_INTM, ~MASK_M_SYNC_CHG);
	//} else {
		//743 i2c_wr8(sd, SYS_INTM, ~MASK_M_DDC &0xff);
		//743 i2c_wr8(sd, CLK_INTM,0xff);
		//743 i2c_wr8(sd, CBIT_INTM,0xff);
		//743 //AUD i2c_wr8(sd, AUDIO_INTM,0xff);
		//743 i2c_wr8(sd, MISC_INTM,0xff);
	//}
}
...
static void tc358748_csi_err_int_handler(struct v4l2_subdev *sd, bool *handled)
{
	...
	printk("TC358748DEV: tc358748_csi_err_int_handler : CSI_ERR = %u ", i2c_rd32(sd, CSI_ERR));
	
	//743 i2c_wr32(sd, CSI_INT_CLR, MASK_ICRER);
}
...
static void tc358748_hdmi_misc_int_handler(struct v4l2_subdev *sd, bool *handled)
{
	...
	//printk("TC358748DEV: tc358748_hdmi_misc_int_handler : misc_int_mask = %u ", misc_int_mask);
	//printk("TC358748DEV: tc358748_hdmi_misc_int_handler : misc_int = %u ", misc_int);

	//743 i2c_wr8(sd, MISC_INT, misc_int);
...
static void tc358748_hdmi_cbit_int_handler(struct v4l2_subdev *sd, bool *handled)
{
	...
	//printk("TC358748DEV: tc358748_hdmi_cbit_int_handler : cbit_int_mask = %u ", cbit_int_mask);
	//printk("TC358748DEV: tc358748_hdmi_cbit_int_handler : cbit_int = %u ", cbit_int);

	//743 i2c_wr8(sd, CBIT_INT, cbit_int);

	v4l2_info(sd, "%s: CBIT_INT =0x%02x\n", __func__, cbit_int);

	if (cbit_int & MASK_I_CBIT_FS) {

		//AUD v4l2_info(sd, "%s: Audio sample rate changed\n", __func__);
		//AUD tc358748_s_ctrl_audio_sampling_rate(sd);
	...
	if (cbit_int & (MASK_I_AF_LOCK | MASK_I_AF_UNLOCK)) {
		...
		//AUD v4l2_info(sd, "%s: Audio present changed\n", __func__);
		//AUD tc358748_s_ctrl_audio_present(sd);
...
static void tc358748_hdmi_clk_int_handler(struct v4l2_subdev *sd, bool *handled)
{
	...
	//printk("TC358748DEV: tc358748_hdmi_clk_int_handler : clk_int_mask = %u ", clk_int_mask);
	//printk("TC358748DEV: tc358748_hdmi_clk_int_handler : clk_int = %u ", clk_int);

	/* Bit 7 and bit 6 are set even when they are masked */
	//743 i2c_wr8(sd, CLK_INT, clk_int |0x80 | MASK_I_OUT_H_CHG);
...
static void tc358748_hdmi_sys_int_handler(struct v4l2_subdev *sd, bool *handled)
{
	...
	//printk("TC358748DEV: tc358748_hdmi_sys_int_handler : sys_int_mask = %u ", sys_int_mask);
	//printk("TC358748DEV: tc358748_hdmi_sys_int_handler : sys_int = %u ", sys_int);

	//743 i2c_wr8(sd, SYS_INT, sys_int);
	...
	if (sys_int & MASK_I_HDMI) {
		...
		//743 i2c_wr8(sd, ANA_CTL, MASK_APPL_PCSX_NORMAL | MASK_ANALOG_ON);
			                   ...
			                   struct v4l2_dbg_register *reg)
{
	...
	// no return i2c_rd(sd, reg->reg, (u8 *)&reg->val, reg->size);
	return 0;
}
			             ...
			             const struct v4l2_dbg_register *reg)
{
	...
	//743 i2c_wr(sd, (u16)reg->reg, (u8 *)&reg->val,
...
static int tc358748_isr(struct v4l2_subdev *sd, u32 status, bool *handled)
{
	...
	printk("TC358748DEV: tc358748_isr : intstatus = %u ", intstatus);
	v4l2_info(sd, "%s: IntStatus =0x%04x\n", __func__, intstatus);
/*
	if (intstatus & MASK_HDMI_INT) {
		u8 hdmi_int0 = i2c_rd8(sd, HDMI_INT0);
		u8 hdmi_int1 = i2c_rd8(sd, HDMI_INT1);
		printk("TC358748DEV: tc358748_isr point1 : hdmi_int0 = %u ", hdmi_int0);
		printk("TC358748DEV: tc358748_isr point1 : hdmi_int1 = %u ", hdmi_int1);
		...
		//743 i2c_wr16(sd, INTSTATUS, MASK_HDMI_INT);
		intstatus &= ~MASK_HDMI_INT;
	}

	if (intstatus & MASK_CSI_INT) {
		u32 csi_int = i2c_rd32(sd, CSI_INT);
		printk("TC358748DEV: tc358748_isr point2 : csi_int = %u ", csi_int);

		if (csi_int & MASK_INTER)
			tc358748_csi_err_int_handler(sd, handled);

		//743 i2c_wr16(sd, INTSTATUS, MASK_CSI_INT);
		intstatus &= ~MASK_CSI_INT;
	}

	intstatus = i2c_rd16(sd, INTSTATUS);
	*/
	printk("TC358748DEV: tc358748_isr point 3: intstatus = %u ", intstatus);
	if (intstatus) {
		v4l2_info(sd,
...
{
	struct tc358748_state *state = to_state(sd);
	u8 vi_rep = 96; //i2c_rd8(sd, VI_REP);
		                   ...
		                   struct v4l2_subdev_edid *edid)
{
	...
	//HC no return i2c_rd(sd, EDID_RAM + (edid->start_block * EDID_BLOCK_SIZE), edid->edid, edid->blocks * EDID_BLOCK_SIZE);
				           ...
				           struct v4l2_subdev_edid *edid)
{
	...
	//743 i2c_wr8(sd, EDID_LEN1, edid_len &0xff);
	//743 i2c_wr8(sd, EDID_LEN2, edid_len >> 8);

	if (edid->blocks == 0) {
		state->edid_blocks_written = 0;
		return 0;
	}
	//743 i2c_wr(sd, EDID_RAM, edid->edid, edid_len);
	/* richardyou
...
//	V4L2_DV_BT_CEA_1920X1080P60;
	...
	tc358748_enable_interrupts(sd, true);
	//743 i2c_wr16(sd, INTMASK, ~(MASK_HDMI_MSK | MASK_CSI_MSK) &0xffff);
	...
	//tc358746_set_buffers
	i2c_wr16(sd, FIFOCTL, pdata->fifo_level);
	// VWCnt(wordcnt) is disable.
	...
	//tc358746_set_csi_color_space
	i2c_wr16_and_or(sd, DATAFMT,~(DATAFMT_PDFMT_MASK | DATAFMT_UDT_EN_MASK),DATAFMT_PDFMT_SET(0x6));
	i2c_wr16_and_or(sd, CONFCTL, ~CONFCTL_PDATAF_MASK,CONFCTL_PDATAF_SET(0x0));
	//i2c_wr16_and_or(sd, DATAFMT,~(DATAFMT_PDFMT_MASK | DATAFMT_UDT_EN_MASK),DATAFMT_PDFMT_SET(tc358746_fmt->pdformat));
	//i2c_wr16_and_or(sd, CONFCTL, ~CONFCTL_PDATAF_MASK,CONFCTL_PDATAF_SET(tc358746_fmt->pdataf));
	
	//tc358746_sleep_mode
	i2c_wr16_and_or(sd, SYSCTL, ~SYSCTL_SLEEP_MASK,SYSCTL_SLEEP_MASK);
	
	//tc358746_set_pll
	tc358748_set_pll(sd);
	
	//tc358746_enable_stream
	i2c_wr16_and_or(sd, PP_MISC, ~PP_MISC_FRMSTOP_MASK, PP_MISC_FRMSTOP_MASK);
