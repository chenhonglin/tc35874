...
static int tc358748_apply_fw(struct tc358748_state *state)
{
	struct tc358748_csi_param *csi_setting;
	int err, i;
	...
	for (i = 0; i < state->link_frequencies_num; i++) {
		...
		err = tc358748_calculate_csi_txtimings(state, csi_setting);
		if (err) {
...
static int tc358748_probe_fw(struct tc358748_state *state)
{
	struct device *dev = &state->i2c_client->dev;
	...
	ret = tc358748_set_lane_settings(state, &endpoint);
	if (ret)
		goto free_ep;
...
static int tc358748_parse_endpoint(struct device *dev,
			  ...
			  const struct i2c_device_id *id)
{
	...
	/* platform data */
	err = tc358748_probe_fw(state);
	if (err)
		return err;

	err = tc358748_apply_fw(state);
	if (err)
		return err;
...
static const struct of_device_id __maybe_unused tc358748_of_match[] = {
